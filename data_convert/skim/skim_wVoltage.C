...
cout << type << endl;
cout << date << endl;
cout << Form("/data/chocula/villaa/PhotoN_SuperSim/ZipSum/skim_%s/%s_z%d_%s_%s_%s.root",setID.c_str(),type.c_str(),zip,date,setID.str().c_str(),suffix.c_str()) << endl;
Int_t nev;
...
ch->Process(test);
TFile *fout = new TFile(Form("/data/chocula/villaa/PhotoN_SuperSim/ZipSum/skim_%s/%s_z%d_%s_%s_%s.root",setID.c_str(),type.c_str(),zip,date,setID.str().c_str(),suffix.c_str()),"RECREATE");
